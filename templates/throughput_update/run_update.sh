...
if [ $# -ne 7 ]; then
  echo "please use: $0 result_dir ip port dbname user pwd stream_count"
  echo "or"
...
function benchmark_dss() {
	...
	print_log "running updates defined in TPC-H benchmark"

	for n in `seq 1 $STREAM_COUNT`
	do

		print_log "  running stream $n"

		u1="dss/updates/rf1_u$n.sql"
