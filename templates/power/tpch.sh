...
function benchmark_dss() {
	...
	u1="dss/updates/rf1_u1.sql"
        print_log "  running rf1 sql $n"
        /usr/bin/time -a -f "rf1_u1 = %e" -o $RESULTS/results.log psql -h $IP -p $PORT -U $USER $DBNAME < $u1 > $RESULTS/results/rf1_u1 2> $RESULTS/errors/rf1_u1
