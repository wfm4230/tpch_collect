...
*/
	...
	function load_queries($dir) {
		...
		foreach ($results AS $line) {
			...
			if ((substr_count($line, '=') > 0) && (!(strpos($line, "rf") === 0))) {
	...
	function parse_log(&$data, $logfile) {
		...
		for ($i = 0; $i < count($log); $i++) {
			$matches = array();
		    if (preg_match('/[0-9][0-9]:[0-9][0-9]:[0-9][0-9] \[([0-9]+)\] : running TPC-H benchmark/', $log[$i], $matches)) {
				$t = intval($matches[1]);
